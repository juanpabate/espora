...
export async function loader({ request, params }: LoaderFunctionArgs) {
  const { postId, userId: postOwnerId } = params;

  if (!postId || !postOwnerId) return redirect("/inicio");
...
export default function PostGallery() {
  ...
  const optimisticLike = likeFetcher.formData
    ...
    : userLoged?.likedPostIds.includes(post.id);
  ...
  const optimisticSave = saveFetcher.formData
    ...
    : userLoged?.savePostIds.includes(post.id);
